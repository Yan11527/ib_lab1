...
- Получать данные пользователей через REST API (JSON)
...
**Ответ:**
- **200 OK**: JSON с данными пользователей
```json
{
  "users": [
    {"id": 1, "username": "user1"},
...
2. **API запросы**: Проверка токена в заголовке `Authorization: Bearer <token>` или URL параметре `?token=<token>`
3. **Веб-интерфейс**: Проверка токена в URL параметре `?token=<token>`
4. **Валидация**: Проверка подписи и времени жизни токена
5. **Обработка ошибок**: API endpoints возвращают 401 с JSON, веб-страницы - редирект на страницу ошибки
...
**Защищенные пути:**
- `/api/*` (кроме `/api/auth/login`) - возвращают 401 JSON при отсутствии токена
- `/data` - редирект на страницу ошибки при отсутствии токена

**Защита:**
- Автоматическая проверка всех защищенных эндпоинтов через `JwtFilter`
- Поддержка двух способов передачи токена: заголовок `Authorization: Bearer <token>` или URL параметр `?token=<token>`
